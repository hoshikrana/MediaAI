...
class ImageValidator:
    ...
    async def validate(file: UploadFile) -> ImageMetadata:
        ...
        if len(content) > max_size:
            raise FileTooLargeError(
                f"Max file size is {settings.MAX_UPLOAD_SIZE_MB}MB"
            )
        await file.seek(0)
        ...
        if mime not in ImageValidator.ALLOWED_MIME_TYPES:
            raise InvalidFileTypeError(
                f"File type '{mime}' not supported. Allowed: "
                f"{', '.join(ImageValidator.ALLOWED_MIME_TYPES)}"
            )
        ...
        try:
            ...
            raise InvalidFileError(
                "File is corrupted or cannot be read as an image"
            ) from exc

        if (width < ImageValidator.MIN_DIMENSION or
                height < ImageValidator.MIN_DIMENSION):
            raise InvalidFileError(
                f"Image too small (min {ImageValidator.MIN_DIMENSION}px)"
            )
        if (width > ImageValidator.MAX_DIMENSION or
                height > ImageValidator.MAX_DIMENSION):
            raise InvalidFileError(
                f"Image too large (max {ImageValidator.MAX_DIMENSION}px)"
            )
    ...
    def _validate_chest_xray_like(img: Image.Image) -> None:
        ...
        This is intentionally conservative: it is not a diagnosis model, only
        a safety gate to prevent normal photos/screenshots from receiving
        fake-looking medical scores.
        """
        ...
        try:
            ...
            if cascade_path.is_file():
                ...
                if len(faces) > 0:
                    raise InvalidFileError(
                        "This appears to be a person/photo, not a chest "
                        "X-ray. Please upload a medical radiograph."
                    )
        except InvalidFileError:
            raise
        except Exception as exc:
